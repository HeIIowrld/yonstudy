"""rclone을 이용한 OneDrive 직접 저장.

새 첨부파일은 로컬 staging 파일을 만들지 않고 메모리에서 ``rclone rcat``으로
전송한다. SQLite에는 재시도와 증분 동기화에 필요한 원격 경로, 크기, 해시만 남긴다.
"""
...
class DirectSyncResult:
    destination: str
    mode: str = "direct-no-local-staging"
    courses: int = 0
...
class RcloneOneDrive:
    """OneDrive의 현재 학기 목록을 한 번 읽고 바이트를 직접 전송한다."""
    ...
    def __init__(self, remote: str, *, timeout: int = 6 * 3600):
        ...
        if not self.exe:
            raise OneDriveError("rclone이 설치되어 있지 않습니다")
        ...
        try:
            ...
            raise OneDriveError("rclone listremotes 시간 초과") from exc
        if proc.returncode != 0:
            raise OneDriveError((proc.stderr or proc.stdout).strip() or "rclone 설정 조회 실패")
        if remote_name not in set(proc.stdout.splitlines()):
            raise OneDriveError(f"rclone remote가 설정되어 있지 않습니다: {remote_name}")
    ...
    def _load_term(self, term: str) -> dict[str, int]:
        ...
        if proc.returncode != 0:
            ...
            if "directory not found" in message.lower() or "not found" in message.lower():
                listing: dict[str, int] = {}
            else:
                raise OneDriveError(message or f"OneDrive 목록 조회 실패: {term}")
        else:
            try:
                items = json.loads(proc.stdout or "[]")
            except json.JSONDecodeError as exc:
                raise OneDriveError(f"OneDrive 목록 응답을 해석하지 못했습니다: {term}") from exc
            listing = {
    ...
    def upload_bytes(self, relative: str, body: bytes, *, force: bool = False) -> bool:
        ...
        try:
            ...
            raise OneDriveError(f"OneDrive 업로드 시간 초과: {relative}") from exc
        if proc.returncode != 0:
            output = (proc.stdout + b"\n" + proc.stderr).decode("utf-8", "replace").strip()
            raise OneDriveError(output[-2000:] or f"OneDrive 업로드 실패: {relative}")
        if "/" in relative:
            term = relative.split("/", 1)[0]
    ...
    def upload_file(self, relative: str, source: str | Path, *, force: bool = False) -> bool:
        """큰 파일을 메모리에 올리지 않고 OneDrive로 직접 전송한다."""
        relative = str(PurePosixPath(relative))
        source = Path(source)
    ...
    def move(self, source: str, target: str, *, size: int | None = None) -> bool:
        ...
        try:
            ...
            raise OneDriveError(f"OneDrive 파일 이동 시간 초과: {source}") from exc
        if proc.returncode != 0:
            output = (proc.stdout + b"\n" + proc.stderr).decode("utf-8", "replace").strip()
            raise OneDriveError(output[-2000:] or f"OneDrive 파일 이동 실패: {source}")
...
def sync_onedrive_tree(store, sink: RcloneOneDrive, *, year: str, semester: str) -> DirectSyncResult:
    """DB의 현재 학기 자료와 글을 로컬 export 없이 OneDrive와 맞춘다."""
    result = DirectSyncResult(destination=sink.remote)
    courses = store.query(
        """
