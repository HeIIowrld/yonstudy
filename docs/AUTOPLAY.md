# 자동수강 — 진도 처리 방식 분석과 구현

> **먼저 짚습니다.** 자동 재생으로 수강 기록을 미리 만들어 두는 건 학칙상 부정수강으로
> 해석될 소지가 있고, 규정 위반 시 책임은 계정 주인에게 갑니다.
> 이 구현은 **실제로 영상을 재생해 LMS가 자연스럽게 기록하는 방식만** 다룹니다.
> 진도 로그를 직접 POST로 위조하거나 재생 없이 시청시간을 만드는 경로는 넣지 않았습니다.
...
서버로 가는 페이로드에 **경과 실시간(wall clock)이 들어가지 않는다.**
...
| 신호 | 의미 |
|---|---|
| `isProgressPeriodCheck` (`progress_period`) | **false면 `ajax()` 자체가 no-op** — 진도처리기간이 아니면 아무리 재생해도 기록되지 않는다 |
...
번들의 `c.progress=function(d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,A,B,C)` 와 1:1 대응:
...
2. **devtools 탐지 트랩이 있다.**
   ...
   → **Playwright의 `page.on("console")` 구독 금지.**
3. **`beforeunload`에서 확인창을 띄운다** → `page.on("dialog", d => d.accept())` 필요.
...
`build_plan(store)` 선정 조건 (둘 다 만족):

1. 진도율 100%와 최대 학습 위치 완주 조건을 모두 충족하지 않음
2. 뷰어에서 읽은 `progress_period`가 참이고 강좌 페이지의 **정상 수강기간** 안

지각기한은 리포트에 표시하지만 기본 자동재생에는 포함하지 않는다.

우선순위: `긴급`(D-1) → `임박`(D-3) → `여유` → `상시`, 같은 등급이면 마감이 이른 순.

실측 검증 (2026-03-08 시점 재현):
...
- Playwright Chromium의 격리 context, `--mute-audio` (H.264/AAC 실행 전 확인)
