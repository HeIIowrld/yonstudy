...
│   ├── store.py             SQLite, SHA-256 blob, 스키마 마이그레이션
...
│   ├── recordings.py        시간표 가져오기, 녹음 시각 판정과 과목 자동 분류
│   └── studykit/
│       ├── slides.py         PDF 슬라이드 추출, 강의안 후보 탐색
│       ├── align.py          TF-IDF, DTW 정렬, 핫워드 추출, 언어 감지
...
├── blobs/ab/cd/abcd…               파일 실체. 중복은 하나만 저장
...
같은 파일이 여러 강좌나 역할에 걸쳐 있어도 `blob`은 SHA-256 해시 하나만 남고 강좌 폴더에는
하드링크만 건다.
...
남지만 현재 리포트, 대기열, 자동 재생 후보에서는 제외된다. HTML 일부만 파싱된 경우에는
...
`assign`만 있는 게 아니라 `turnitintooltwo`(표절 검사), `vpl`(코딩), `quiz`, `feedback`,
...
"콘텐츠가 실제로 없음"이 구분되지 않는다. 이 때문에 94편을 파서 실패로 오해했다.

## 스키마 마이그레이션

`CREATE TABLE IF NOT EXISTS`는 기존 테이블에 컬럼을 추가하지 않는다.
`Store._migrate()`가 `SCHEMA`를 파싱해 실제 테이블에 없는 컬럼을 `ALTER TABLE ADD COLUMN`으로
붙인다. 컬럼 추가만 지원하며, 타입 변경·삭제는 다루지 않는다.

## 동시성

SQLite를 WAL 모드와 `busy_timeout=30000` 설정으로 연다. 긴 아카이빙을 백그라운드로 돌리면서
`status`와 `analyze`를 동시에 실행하는 일이 흔하기 때문이다.
`Store.log()`는 쓰기 충돌 시 조용히 건너뛴다. 진단 로그 한 줄 때문에 긴 작업이 중단되어서는 안 된다.
