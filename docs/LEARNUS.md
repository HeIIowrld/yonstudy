...
과제 명세는 정규식으로 태그를 지우는 대신 `html_content.py`의 HTML 트리로 변환한다.
이미지만 있는 본문도 수집하고, 번호 목록·중첩 목록·표·코드 공백·링크·상대 이미지
주소를 보존한다. OJ 본문 안의 가로줄은 내용 경계로 취급하지 않으며 설명 뒤의
clarification 버튼만 제외한다. 코드 언어가 없는 예제에는 Python 표시를 붙이지 않는다.

Gradescope의 제출 후 화면은 `AssignmentSubmissionViewer`의 `outline`과 `questions`를
문항 ID로 결합한다. 저장 대상은 공개 문항·배점·제출 안내이며 답안·성적 상세·명단은
포함하지 않는다. 과제의 LeetCode 링크는 별도 비인증 클라이언트로 공개 문제 명세와
Python 3 시작 코드를 가져온다. 일부 외부 조회가 실패하면 기존 수집본과 실패 사유를
남기며 다른 문항 수집은 계속한다.

`.ipynb` 과제 첨부는 10 MiB 이하의 nbformat 4 문서에서 Markdown 셀과 코드 셀의
source만 순서대로 추출한다. 출력·실행 횟수·메타데이터는 제외한다. HTML 내보내기에는
설명 셀을 제목·목록·강조·링크·표로 표시하고 코드 셀은 들여쓰기를 유지한다. 기존에
저장된 Markdown 미리보기도 내보낼 때 읽기 화면으로 변환한다. 추출용 첨부는 로컬 blob에 보존하므로 이후
`--no-files` 조회에서도 명세를 유지할 수 있다. PDF·DOCX는 첨부 링크와 원본을 보존한다.
